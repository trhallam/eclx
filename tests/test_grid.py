...
def test_open_EclGrid_bad_file(eclipse_runs):
    filepath = eclipse_runs.with_suffix(".DATA")
...
def test_load_ecl_grid_index(eclipse_runs):
    result = load_ecl_grid_index(eclipse_runs.with_suffix(".EGRID"))
    assert result.shape == (75, 5)


def test_load_ecl_grid(eclipse_runs):
    result = load_ecl_grid(eclipse_runs.with_suffix(".EGRID"))
